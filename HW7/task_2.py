...
# Здесь пишем код
class PersonInfo:
    def __init__(self, name, age, *division: str):
        self.name = name
        self.age = age
        self.division = division

    def short_name(self):
        return self.name[self.name.index(' ') + 1:] + ' ' + self.name[:self.name.index(' ')][0] + '.'

    def path_deps(self):
        return ' --> '.join(self.division)

    def new_salary(self):
        text = ''.join(self.division)
        count = sorted([(text.count(letter), letter) for letter in set(text)], reverse=True)
        salary = 1337 * self.age * (count[0][0]+count[1][0]+count[2][0])
        return salary
